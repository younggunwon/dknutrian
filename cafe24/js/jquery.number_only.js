 ...
 * @param integer decimalPart 소수점 자리 수
 * <code>
 ...
 */
(function($){
	$.fn.number_only = function(stringLength, maxNum, defaultNum, decimalPart) {
		/* 숫자만입력. sj */
		var onlyDigit = false;
		...
		return this.each(function() {
			$(this).keydown(function(event){
				...
				if (((event.keyCode == 110 || event.keyCode == 190) && newText.lastIndexOf(".") > -1) || ((event.keyCode == 109 || event.keyCode == 189) && newText.lastIndexOf("-") > -1)) {
					return false; // . 과 -는 한 번 이상 입력 불가
				}
				// 숫자만입력. sj
...
//				var newText	= oldText.replace(/[^0-9,.-]+/g,"");
				...
				if((event.keyCode == 109 || event.keyCode == 189) && newText.lastIndexOf("-") > 0) { // -는 맨 앞에만 입력 가능
					newText = newText.replace('-', '');
				}
				if(decimalPart > 0 && newText.indexOf('.') != -1) { // 소수점 버림 처리
					var part = newText.split('.');
					if(part[1].length > decimalPart) {
						newText = part[0] + '.' + part[1].substring(0, decimalPart);
					}
				}
